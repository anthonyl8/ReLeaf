...
class AnalysisService:
    ...
    def get_hotspots(self) -> list[dict]:
        ...
        # Get bounds to scan the entire thermal coverage area
        bounds = satellite_service.get_bounds()
        if not bounds:
            return []

        # Sample grid points across the coverage area
        lat_min = bounds.get('lat_min', settings.DEFAULT_LAT - 0.02)
        lat_max = bounds.get('lat_max', settings.DEFAULT_LAT + 0.02)
        lon_min = bounds.get('lon_min', settings.DEFAULT_LON - 0.02)
    ...
    async def get_vulnerability_data(self) -> list[dict]:
        ...
        if len(vulnerability) < 3:
            ...
            bounds = satellite_service.get_bounds()
            if bounds:
                lat_min = bounds.get('lat_min', center_lat - 0.01)
                lat_max = bounds.get('lat_max', center_lat + 0.01)
                lon_min = bounds.get('lon_min', center_lon - 0.01)
                lon_max = bounds.get('lon_max', center_lon + 0.01)
